...
/*** シミュレーションループ ***/
  ...
  double bb = 1.35;// 数値は 1.3 から 2.0 の間が良い
  double cc = 0.7;// 0 < beta <1 で、 0.5 から 0.8 が良い
  double dd = 1.05;// 0.7 から 1.5 の間が良い

//  他のパラメータ設定
  double minn = pi/2.0;
  double maxx = 3.0*pi/2.0;
  double df = (maxx-minn)/100.0;// 位相角 f のプロット間隔
  double f = minn;
  double t,r;
...
void simLoop(int pause)
{
  ...
  t=(1-bb)*(f-pi/2)+2*bb*asin((f-pi/2)/pi)+pi/2;
  r=aa*(1-sin(t));
  P[0] = 1;
  P[1] = r*(1+cc*sin(f))*cos(f)+1.0;
  P[2] = dd*r*(1+cc*sin(f))*sin(f)+1.0;
  f += df;
//  定数設定
