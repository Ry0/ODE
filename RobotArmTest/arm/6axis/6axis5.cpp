...
dReal P[3] = {1.0, 1.0, 1.0};             // 先端の位置
// 有顔ベクトル(a,b)
dReal a[3] = {1.0, 0.0, 0.0};//?わっからーん
...
void makeSensor()
{
  ...
  double size = 0.02, weight = 0.00001; // センサのサイズ[m]と重量[kg]
...
void command2(int cmd)
{
  switch (cmd) {
    ...
    case 'z':  T[0] += 0.1; break;   // kキーを押すと先端のy座標が増加
    case 'v':  T[0] -= 0.1; break;   // dキーを押すと先端のy座標が減少
    case 'x':  T[1] += 0.1; break;   // lキーを押すと先端のz座標が増加
    case 'c':  T[1] -= 0.1; break;   // sキーを押すと先端のz座標が減少
  }
}
