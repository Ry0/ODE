...
double T[2] = {M_PI, 0.0};
double THETA[7] = {0.0};  // 関節の目標角度[rad]
...
void CheckThetaE(double j)
{
  ...
  //   CheckAnswer = 0;
  //   CalTheta[2] = (double)j*M_PI/180.0;
  //   inverseKinematics();
  //   for (int i = 0; i < 7; ++i){
  //     if(min_theta[i] <= CalTheta[i] && CalTheta[i] <= max_theta[i]){
  //       CheckAnswer += 1;
